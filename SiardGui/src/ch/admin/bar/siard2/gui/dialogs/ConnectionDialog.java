...
/*====================================================================*/
/**
 * ConnectionDialog for entering data to connect to a database. Abstract base
 * class for DownloadConnectionDialog and UploadConnectionDialog.
 * 
...
{
	/** logger */
	private static IndentLogger		_il					= IndentLogger
	        .getIndentLogger(ConnectionDialog.class.getName());
	// width of JDBC URL input box
	// protected static final double dWIDTH_URL =
	// FxSizes.getScreenBounds().getWidth() / 2.0;
	protected static final double	dWIDTH_URL			= 700;
...
//	TableColumn colname1 	= new TableColumn("Table Name");
//    TableColumn colname2 	= new TableColumn("Check");
	...
	private class ActionEventHandler implements EventHandler<ActionEvent>
	{
		...
		public void handle(ActionEvent ae)
		{
			...
			else if (ae.getSource() == _btnDbFolder)
			{
				...
				try
				{
					fileDbFolder = FS.chooseExistingFolder(ConnectionDialog.this, sb.getConnectionDbFolderTitle(),
					        sb.getConnectionDbFolderMessage(), sb, fileDbFolder);
	...
	private class StringChangeListener implements ChangeListener<String>
	{
		 ...
		 */
		@Override
		public void changed(ObservableValue<? extends String> ovs, String sOld, String sNew)
		{
			if (!_bInListener)
			{
				_bInListener = true;
				...
				if ((ovs == _cbDbScheme.getSelectionModel().selectedItemProperty()) || (ovs == _tfDbHost.textProperty())
				        || (ovs == _tfDbFolder.textProperty()) || (ovs == _tfDbName.textProperty()))
				{
					if (_tfConnectionUrl != null)
					{
						int iSelectedOption = getSelectedOption();
						String sSampleUrl = sc.getSampleUrl(sScheme, _tfDbHost.getText(), _tfDbFolder.getText(),
						        _tfDbName.getText(), iSelectedOption);
	...
	/* ==================================================================== */
	/**
	 * PastingTextField implements non-editable field which can receive paste events
	 * and serve as a drop target.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * compute the maximum pref width of the given labels and set their min and pref
	 * widths to that value.
	 * 
	 ...
	 * @return toggle group, or null, if no toggle group is associated with the
	 *         scheme.
	 */
	...
	protected HBox createHBox(Label lbl, PastingTextField ptfText, Button btn)
	{
		...
		hbox.setMinWidth(lbl.getPrefWidth() + dHSPACING + ptfText.getPrefWidth() + dHSPACING + btn.getPrefWidth());
		return hbox;
	} /* createHBox */
	 ...
	 */
	protected VBox createVBoxConnectionParameters(String sLoadMetaDataOnlyLabel, String sLoadMetaDataOnlyTooltip,
	        String sLoadOverwriteLabel, String sLoadOverwriteTooltip, String sLoadViewsAsTablesLabel,
	        String sLoadViewsAsTablesTooltip)
	{
		SiardBundle sb = SiardBundle.getSiardBundle();
		UserProperties up = UserProperties.getUserProperties();
...
//		_tvSelectTable.setItems(rowdata);
		...
		double dMinWidth = getMaxPaneMinWidth(hboxDbUser, hboxDbPassword, hboxMetaDataOnly, hboxOverwrite,
		        hboxViewsAsTables);
		vbox.setMinWidth(dMinWidth);
		return vbox;
	 ...
	 * @return main VBox
	 */
	private VBox createVBoxDialog(String sLoadMetaDataOnlyLabel, String sLoadMetaDataOnlyTooltip,
	        String sLoadOverwriteLabel, String sLoadOverwriteTooltip, String sLoadViewsAsTablesLabel,
	        String sLoadViewsAsTablesTooltip)
	{
		VBox vbox = new VBox();
		...
		VBox vboxConnectionParameters = createVBoxConnectionParameters(sLoadMetaDataOnlyLabel, sLoadMetaDataOnlyTooltip,
	 ...
	 * @param sConnectionUrl            initial value for connection (JDBC) URL or
	 *                                  null.
	 ...
	 * @param sLoadViewsAsTablesTooltip tool tip for views as tables check box.
	 */
	protected ConnectionDialog(Stage stageOwner, String sConnectionUrl, String sDbUser, String sTitle,
	        String sLoadMetaDataOnlyLabel, String sLoadMetaDataOnlyTooltip, String sLoadOverwriteLabel,
	        String sLoadOverwriteTooltip, String sLoadViewsAsTablesLabel, String sLoadViewsAsTablesTooltip)
	{
		...
		VBox vboxDialog = createVBoxDialog(sLoadMetaDataOnlyLabel, sLoadMetaDataOnlyTooltip, sLoadOverwriteLabel,
		        sLoadOverwriteTooltip, sLoadViewsAsTablesLabel, sLoadViewsAsTablesTooltip);
