...
public class OpenSaveAction
{
	...
	/*------------------------------------------------------------------*/
	/**
	 * if sFile is null or does not exist, display file selector, otherwise open the
	 * file.
	 * 
	...
	public void open(String sFile)
	{
		...
		if (fileArchive == null)
		{
			fileArchive = new File(SiardGui.getDefaultDataDirectory().getAbsolutePath() + File.separator + "*."
					+ Archive.sSIARD_DEFAULT_EXTENSION);
			if (MruFile.getMruFile().getMruFiles() > 0)
				fileArchive = (new File(MruFile.getMruFile().getMruFile(0)));
			try
			{
				fileArchive = FS.chooseExistingFile(stage, sb.getOpenArchiveTitle(), sb.getOpenArchiveMessage(), sb,
		...
		if (fileArchive != null)
		{
			...
			try
			{
				...
				{
					MB.show(stage, sb.getOpenErrorTitle(), sb.getOpenErrorInvalidMessage(fileArchive), sb.getOk(),
							null);
				}
			}
			catch (IOException ie)
			{
				MB.show(stage, sb.getOpenErrorTitle(), sb.getOpenErrorMessage(fileArchive, ie), sb.getOk(), null);
			}
	...
	{	
		...
		{
			MB.show(SiardGui.getSiardGui().getStage(), sb.getSaveErrorTitle(),
					sb.getSaveErrorMessage(archive.getFile(), ie), sb.getOk(), null);
		}
		SiardGui.getSiardGui().terminateAction();
		_il.exit();
	}
