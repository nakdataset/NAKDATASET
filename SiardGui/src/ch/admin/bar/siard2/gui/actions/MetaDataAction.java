 ...
 */
public class MetaDataAction implements FileFilter
{
	...
	/*------------------------------------------------------------------*/
	/**
	 * search for XSL in etc opening file selector, if there is more than one, and
	 * display meta data dialog using it.
	 */
	...
	public void augmentMetaData()
	{
		...
		File fileMetaData = new File(
				up.getImportMetadataFolder().getAbsolutePath() + File.separator + "*." + sXML_EXTENSION);
		try
		{
			fileMetaData = FS.chooseExistingFile(stage, sb.getMetaDataAugmentTitle(), sb.getMetaDataAugmentMessage(),
					sb, fileMetaData, sXML_EXTENSION);
		}
		...
		if (fileMetaData != null)
		{
			...
			{
				MB.show(SiardGui.getSiardGui().getStage(), sb.getMetaDataErrorTitle(),
						sb.getMetaDataErrorAugmentMessage(fileMetaData, ie), sb.getOk(), null);
			}
		}
		_il.exit();
	} /* augmentMetaData */
	...
	/*------------------------------------------------------------------*/
	/**
	 * save the given meta data string to the file system. Display save file dialog
	 * for given extension and save meta data to selected file.
	 * 
	 ...
	 */
	private void saveMetaData(Stage stageOwner, Archive archive, String sMetaData, String sExtension)
	{
		_il.enter();
		SiardBundle sb = SiardBundle.getSiardBundle();
		Stage stage = SiardGui.getSiardGui().getStage();
		UserProperties up = UserProperties.getUserProperties();
		File fileMetaData = new File(
				up.getExportMetadataFolder().getAbsolutePath() + File.separator + "*." + sExtension);
		try
		{
			fileMetaData = FS.chooseNewFile(stage, sb.getMetaDataSaveTitle(), sb.getMetaDataSaveMessage(), sb,
		...
		if (fileMetaData != null)
		{
			...
			{
				MB.show(stageOwner, sb.getMetaDataErrorTitle(), sb.getMetaDataErrorSaveMessage(fileMetaData, ie),
						sb.getOk(), null);
			}
		}
		_il.exit();
