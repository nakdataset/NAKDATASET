...
/*====================================================================*/
/**
 * SiardGui implements the graphical user interface of SIARD Suite 2.x as a
 * JavaFX application.
 * 
 * @author Hartwig Thomas
 */
public class SiardGui extends Application implements EventHandler<WindowEvent>
{
	/** singleton */
	...
	private static final List<String> listCREATORS = Arrays.asList(new String[]{"Hartwig Thomas"});
	...
	private static final List<String> listCONTRIBUTORS = Arrays.asList(new String[]{"Marcel Büchler", "Krystyna Ohnesorge"});
	...
	private static final List<String> listPROVENANCES = Arrays
			.asList(new String[]{"Enter AG, Rüti ZH, Switzerland", "Swiss Federal Archives, Berne, Switzerland"});
	...
	/** info */
	public static ProgramInfo	_pi				= ProgramInfo.getProgramInfo("SIARD Suite", MF.getSpecificationVersion(),
	...
	public void logPerformance()
	{
		...
		_il.info("GUI: Used Memory: " + StopWatch.formatLong(rt.totalMemory() - rt.freeMemory()) + ", Free Memory: "
				+ StopWatch.formatLong(rt.freeMemory()) + ", Open: " + swOpen.formatMs() + ", Valid: "
				+ ((ArchiveImpl) getArchive())._swValid.formatMs());
	} /* logPerformance */
	 ...
	 * 
	 * @param sStatus
	 *            status to be displayed in status line.
	 ...
	 * 
	 * @param sVersion
	 *            version to compare running instance to.
	 * @return 1 for running version greater than given version (or given
	 *         version is null), 0 for running version equals given version, -1
	 *         for running version is less than given version (not null).
	 */
	public static int compareVersion(String sVersion)
	{
		...
		if(sVersion != null)
		{
			...
			for (int iVersionComponent = 0; (iCompare == 0)
					&& (iVersionComponent < Math.min(asInstalled.length, asRunning.length)); iVersionComponent++)
				iCompare = Integer.compare(Integer.parseInt(asRunning[iVersionComponent]),
						Integer.parseInt(asInstalled[iVersionComponent]));
			if(iCompare == 0)
	...
	/*------------------------------------------------------------------*/
	/**
	 * checks whether the current instance is running from a JAR installed in
	 * the given folder.
	 * 
	 * @param fileInstalled
	 *            folder where SIARD Suite is installed.
	 * @return true, if current instance is running from JAR installed in the
	 *         given folder.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * check if running instance is installed instance. N.B.: Otherwise
	 * UserProperties are not stored.
	 * 
	...
	public static boolean isRunningInstalled()
	{
		...
		return ((compareVersion(up.getInstalledVersion(null)) == 0) && isRunningFrom(up.getInstalledPath(null)));
	} /* is RunningInstalled */
	...
	private UserProperties loadProperties()
	{
		...
		} else
		{
			...
			// _stage.setHeight(up.getStageHeight(0.7 *
			// rectScreen.getHeight()));
			_stage.setWidth(1024);
			_stage.setHeight(768);
		}
	...
	private void storeProperties()
	{
		...
		UserProperties up = UserProperties.getUserProperties();
		_il.event("Properties for version " + up.getInstalledVersion(null) + " are being stored to \""
				+ up.getFile().getAbsolutePath() + "\"");
		SiardBundle sb = SiardBundle.getSiardBundle();
	...
	public void closeArchive()
	{
		...
		try
		{
			...
			if(_archive.isValid())
			{
				if(!_archive.isMetaDataUnchanged())
				{
					int iResult = MB.show(getStage(), sb.getCloseTitle(), sb.getCloseSaveQuestion(_archive.getFile()),
							sb.getYes(), sb.getNo());
			...
			} else
			{
				if(!_archive.isMetaDataUnchanged())
				{
					int iResult = MB.show(getStage(), sb.getCloseTitle(), sb.getCloseExportMetaDataQuestion(), sb.getYes(),
							sb.getNo());
					if(iResult == MB.iRESULT_SUCCESS)
					{
		...
		{
			MB.show(getStage(), sb.getCloseErrorTitle(), sb.getCloseErrorMessage(_archive.getFile(), ie), sb.getOk(), null);
		}
		_archive = null;
		setTitle();
		MainMenuBar.getMainMenuBar().restrict();
	...
	public void findNext()
	{
		...
		if(ms != null)
			...
			MB.show(getStage(), sb.getFindTitle(), sb.getFindEndMessage(sFindString), sb.getOk(), null);
		MainMenuBar.getMainMenuBar().restrict();
	} /* findNext */
	...
	public void integrity()
	{
		if(_archive != null)
		{
			try
			{
				...
				MB.show(getStage(), sb.getIntegrityCheckTitle(_archive.getFile()), sResultMessage, sb.getOk(), null);
			} catch (IOException ie)
			{
				_il.exception(ie);
			}
	 ...
	 * 
	 * @param oMetaData
	 *            meta data or RecordExtract object.
	 * @param clsTableData
	 *            class of table data to show.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * collapse currently selected record extract and its parents up to the rows
	 * entry of the table tree item.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * canClose calls the handler of the CloseRequest in order to treat internal
	 * close attempts like external ones.
	 * 
	...
	private boolean canClose()
	{
		...
		if(!inAction())
		{
			...
			if((compareVersion(up.getInstalledVersion(null)) == 0) && isRunningFrom(up.getInstalledPath(null)))
				storeProperties();
			bCanClose = true;
		}
	...
	/*------------------------------------------------------------------*/
	/**
	 * compare current version with installed version and offer installation if
	 * the current version is new or more recent.
	 * 
	 * @return
	 */
		...
		if(iCompare > 0)
		{
			if(sInstalledVersion == null)
			{
				...
				iResult = MB.show(getStage(), sb.getInstalledNoneTitle(),
						sb.getInstalledNoneMessage(up.getFile(), getVersion()), sb.getYes(), sb.getNo());
			} else
			{
	...
	public void start(Stage stage) throws Exception
	{
		_il.enter(stage);
		_stage = stage;
		// load the user properties and display an installation note, if
		// appropriate
		...
		SleeperTask.runSleeperTask(iSplashMs, new EventHandler<WorkerStateEvent>()
		{
			/*----------------------------------------------------------------*/
			/**
			 * this is called after the splash panel has terminated. It finishes
	 ...
	 * 
	 * @param args
	 *            optional file name of SIARD file to be opened.
	 */
