...
<xsl:stylesheet
	...
	<xsl:template match="/siard:siardArchive">
		<head>
			...
			<style>
				...
				dt /* puts dt
				and dd on same line and right-adjusts dt */
				{
				float: left;
				width: 15em;
				text-align: right;
				...
				h1 { margin-left: 0mm; display: block; font-size: 2.0em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h2 { margin-left: 5mm; display: block; font-size: 1.8em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h3 { margin-left: 10mm; display: block; font-size: 1.6em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h4 { margin-left: 15mm; display: block; font-size: 1.4em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h5 { margin-left: 20mm; display: block; font-size: 1.2em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h6 { margin-left: 25mm; display: block; font-size: 1.0em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h7 { margin-left: 30mm; display: block; font-size: 0.9em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h8 { margin-left: 35mm; display: block; font-size: 0.8em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
				h9 { margin-left: 40mm; display: block; font-size: 0.7em; font-weight:
				bold; margin-top: 0; margin-bottom: 0; }
			</style>
		</head>
		<body>
			...
			<dl>
				...
				<dd>
					<xsl:for-each select="siard:messageDigest">
						<xsl:value-of select="siard:digestType" />
						:
						<xsl:value-of select="siard:digest" />
			...
			<h2>Schemas</h2>
			<xsl:apply-templates
				select="siard:schemas/siard:schema" />
			<h2>Users</h2>
			<xsl:apply-templates
				select="siard:users/siard:user" />
			<h2>Roles</h2>
			<xsl:apply-templates
				select="siard:roles/siard:role" />
			<h2>Privileges</h2>
			<xsl:apply-templates
				select="siard:privileges/siard:privilege" />
		</body>
	</xsl:template>
	...
	<xsl:template match="siard:schemas/siard:schema">
		<h3>
			Schema
			<xsl:value-of select="siard:name" />
		</h3>
		<dl>
			<dt>Name</dt>
			<dd>
		...
		<h3>Types</h3>
		<xsl:apply-templates
			select="siard:types/siard:type" />
		<h3>Tables</h3>
		<xsl:apply-templates
			select="siard:tables/siard:table" />
		<h3>Views</h3>
		<xsl:apply-templates
			select="siard:views/siard:view" />
		<h3>Routines</h3>
		<xsl:apply-templates
			select="siard:routines/siard:routine" />
	</xsl:template>
	...
	<xsl:template match="siard:types/siard:type">
		...
		<dl>
			...
			<dt>Category</dt>
			<dd>
				<xsl:value-of select="siard:category" />
			</dd>
			<xsl:choose>
				<xsl:when test="siard:category != 'distinct'">
					<dt>Instantiable</dt>
					<dd>
						<xsl:value-of select="siard:instantiable" />
					</dd>
					<dt>Final</dt>
					<dd>
						<xsl:value-of select="siard:final" />
					</dd>
				</xsl:when>
				<xsl:otherwise>
					<dt>Base</dt>
					<dd>
						<xsl:value-of select="siard:base" />
					</dd>
				</xsl:otherwise>
			</xsl:choose>
			<dt>Description</dt>
		...
		<xsl:if test="siard:category = 'udt'">
			<h5>Attributes</h5>
			<xsl:apply-templates
				select="siard:attributes/siard:attribute" />
		</xsl:if>
	</xsl:template>
		...
		<dl>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<dt>Type schema</dt>
					<dd>
						<xsl:value-of select="siard:typeSchema" />
					</dd>
					<dt>Type name</dt>
					<dd>
						<xsl:value-of select="siard:typeName" />
					</dd>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:if test="siard:cardinality">
				<dt>Cardinality</dt>
				<dd>
					<xsl:value-of select="siard:cardinality" />
				</dd>
			</xsl:if>
			<dt>Description</dt>
			<dd>
	...
	<xsl:template match="siard:tables/siard:table">
		...
		<h5>Columns</h5>
		<xsl:apply-templates
			select="siard:columns/siard:column" />
		...
		<xsl:if test="siard:triggers">
			<h5>Triggers</h5>
			<xsl:apply-templates
				select="siard:triggers/siard:trigger" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="siard:columns/siard:column">
		...
		<dl>
			...
			<xsl:if test="siard:lobFolder">
				<dt>LOB folder</dt>
				<dd>
					<xsl:value-of select="siard:lobFolder" />
				</dd>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="siard:type">
					...
					<xsl:if test="siard:mimeType">
						<dt>MIME type</dt>
						<dd>
							<xsl:value-of select="siard:mimeType" />
						</dd>
					</xsl:if>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:if test="siard:fields">
			<h7>Fields</h7>
			<xsl:apply-templates
				select="siard:fields/siard:field" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template
		match="siard:candidateKeys/siard:candidateKey">
		<h6>
			CandiateKey
			<xsl:value-of select="siard:name" />
		</h6>
	...
	<xsl:template match="siard:foreignKeys/siard:foreignKey">
		...
		<dl>
			...
			<dd>
				<xsl:value-of select="siard:referencedSchema" />
			</dd>
			<dt>Referenced table</dt>
			<dd>
				<xsl:value-of select="siard:referencedTable" />
			</dd>
			<dt>References</dt>
			<dd>
				<xsl:for-each select="siard:reference">
					<xsl:value-of select="siard:column" />
					:
					<xsl:value-of select="siard:referenced" />
					<xsl:if test="position()!=last()">
						<br />
					</xsl:if>
				</xsl:for-each>
			...
			<dt>Delete action</dt>
			<dd>
				<xsl:value-of select="siard:deleteActionAction" />
			</dd>
			<dt>Update action</dt>
			<dd>
	...
	<xsl:template match="siard:triggers/siard:trigger">
		...
		<dl>
			...
			<dt>Action time</dt>
			<dd>
				<xsl:value-of select="siard:actionTime" />
			</dd>
			<dt>Trigger event</dt>
			<dd>
				<xsl:value-of select="siard:triggerEvent" />
			</dd>
			...
			<dd>
				<xsl:value-of select="siard:triggeredAction" />
			</dd>
			<dt>Description</dt>
			<dd>
				<xsl:value-of select="siard:description" />
	...
	<xsl:template match="siard:routines/siard:routine">
		<h4>
			Routine
			<xsl:value-of select="siard:routine" />
		</h4>
		<dl>
			<dt>Name</dt>
			<dd>
	...
	<xsl:template match="siard:roles/siard:role">
		...
		<dl>
			...
			<dt>Admin</dt>
			<dd>
				<xsl:value-of select="siard:admin" />
			</dd>
			<dt>Description</dt>
			<dd>
				<xsl:value-of select="siard:description" />
	...
	<xsl:template match="siard:privileges/siard:privilege">
		...
		<dl>
			...
			<dt>Object</dt>
			<dd>
				<xsl:value-of select="siard:object" />
			</dd>
			<dt>Grantor</dt>
			<dd>
				<xsl:value-of select="siard:grantor" />
			</dd>
			<dt>Grantee</dt>
			<dd>
				<xsl:value-of select="siard:grantee" />
			</dd>
			<dt>Option</dt>
			<dd>
