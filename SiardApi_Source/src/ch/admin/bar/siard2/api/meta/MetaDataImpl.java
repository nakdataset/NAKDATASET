...
public class MetaDataImpl extends MetaSearchImpl implements MetaData
{
						...
						if (uriLobFolder.isAbsolute())
						{
							...
							if (!uriLobFolder.getScheme().equals(_sURI_SCHEME_FILE))
								throw new IllegalArgumentException(
										"Only URIs with scheme \"" + _sURI_SCHEME_FILE + "\" allowed for LOB folder!");
						}
						else
						{
	...
	/**
	 * resolve the given LOB folder URI against the current folder where the SIARD
	 * archive is located.
	 * 
	...
	public MetaPrivilege getMetaPrivilege(String sType, String sObject, String sGrantor, String sGrantee)
	{
		...
		for (int iPrivilege = 0; (mp == null) && (iPrivilege < getMetaPrivileges()); iPrivilege++)
		{
			...
			if (Objects.equals(sType, mpTry.getType()) && Objects.equals(sObject, mpTry.getObject())
					&& Objects.equals(sGrantor, mpTry.getGrantor()) && Objects.equals(sGrantee, mpTry.getGrantee()))
				mp = mpTry;
		}
		return mp;
	} /* getMetaPrivilege */
			...
			throws IOException
	{
		...
		if (getArchive().canModifyPrimaryData())
		{
			if (getMetaPrivilege(sType, sObject, sGrantor, sGrantee) == null)
			{
				...
				if (_saTemplate != null)
				{
					...
					if (ptsTemplate != null)
					{
						...
						for (int iPrivilege = 0; (ptTemplate == null)
								&& (iPrivilege < ptsTemplate.getPrivilege().size()); iPrivilege++)
						{
							PrivilegeType ptTry = ptsTemplate.getPrivilege().get(iPrivilege);
							if (Objects.equals(sType, ptTry.getType()) && Objects.equals(sObject, ptTry.getObject())
			...
			throw new IOException(
					"Privileges can only be created if archive is open for modification of primary data.");
		return mp;
	} /* createMetaPrivilege */
	...
	public boolean isValid()
	{
		...
		if (bValid && (!Archive.sMETA_DATA_VERSION.equals(XU.fromXml(_sa.getVersion()))))
			bValid = false;
		/* a valid archive must have a non-empty dbname */
		if (bValid && (!SU.isNotEmpty(_sa.getDbname())))
			bValid = false;
		/* a valid archive must have a non-empty data owner */
		if (bValid && (!SU.isNotEmpty(_sa.getDataOwner())))
			bValid = false;
		/* a valid archive must have a non-empty data origin time span */
		if (bValid && (!SU.isNotEmpty(_sa.getDataOriginTimespan())))
			bValid = false;
		/* a valid archive must have a non-empty archival date */
		if (bValid && (_sa.getArchivalDate() == null))
			bValid = false;
	...
	{
		return new String[] { getVersion(), getDbName(), getDescription(), getArchiver(), getArchiverContact(),
				getDataOwner(), getDataOriginTimespan(), getLobFolder() == null ? "" : getLobFolder().toString(),
				getProducerApplication(), du.fromGregorianCalendar((GregorianCalendar) getArchivalDate()),
				getClientMachine(), getDatabaseProduct(), getConnection(), getDatabaseUser() };
	} /* getSearchElements */

	/*------------------------------------------------------------------*/
	/**
	 * {@inheritDoc} toString() returns the file name of the archive which is to be
	 * displayed as the label of the root of the tree displaying the archive.
	 */
	@Override
