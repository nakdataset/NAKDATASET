...
public class MetaTableImpl extends MetaSearchImpl implements MetaTable
{
		...
		try
		{
				...
				if (getMetaColumn(sName) == null)
				{
					...
					if (_ttTemplate != null)
					{
						...
						if (ctsTemplate != null)
						{
							...
							for (int iColumn = 0; (ctTemplate == null)
							        && (iColumn < ctsTemplate.getColumn().size()); iColumn++)
							{
			...
			else
			{
				throw new IOException(
				        "New columns can only be created if archive is open for modification of primary data and table is empty.");
			}
	...
	public MetaForeignKey createMetaForeignKey(String sName) throws IOException
	{
			...
			if (getMetaForeignKey(sName) == null)
			{
				...
				if (_ttTemplate != null)
				{
					...
					if (fktsTemplate != null)
					{
						...
						for (int iForeignKey = 0; (fktTemplate == null)
						        && (iForeignKey < fktsTemplate.getForeignKey().size()); iForeignKey++)
						{
		...
		{
			throw new IOException(
			        "New foreign keys can only be created if archive is open for modification of primary data.");
		}
		return mfk;
	...
	public MetaUniqueKey createMetaCandidateKey(String sName) throws IOException
	{
			...
			if (getMetaCandidateKey(sName) == null)
			{
				...
				if (_ttTemplate != null)
				{
					...
					if (fktsTemplate != null)
					{
						...
						for (int iCandidateKey = 0; (uktTemplate == null)
						        && (iCandidateKey < fktsTemplate.getCandidateKey().size()); iCandidateKey++)
						{
		...
		{
			throw new IOException(
			        "New candidaten keys can only be created if archive is open for modification of primary data.");
		}
		return muk;
	...
	public MetaCheckConstraint createMetaCheckConstraint(String sName) throws IOException
	{
			...
			if (getMetaCheckConstraint(sName) == null)
			{
				...
				if (_ttTemplate != null)
				{
					...
					if (cctsTemplate != null)
					{
						...
						for (int iCheckConstraint = 0; (cctTemplate == null)
						        && (iCheckConstraint < cctsTemplate.getCheckConstraint().size()); iCheckConstraint++)
						{
		...
		{
			throw new IOException(
			        "New check constraints can only be created if archive is open for modification of primary data.");
		}
		return mcc;
	...
	public MetaTrigger createMetaTrigger(String sName) throws IOException
	{
			...
			if (getMetaTrigger(sName) == null)
			{
				...
				if (_ttTemplate != null)
				{
					...
					if (ttsTemplate != null)
					{
						...
						for (int iTrigger = 0; (ttTemplate == null)
						        && (iTrigger < ttsTemplate.getTrigger().size()); iTrigger++)
						{
		...
		{
			throw new IOException(
			        "New triggers can only be created if archive is open for modification of primary data.");
		}
		return mt;
	} /* createMetaTrigger */
	...
	protected MetaSearch[] getSubMetaSearches() throws IOException
	{
		...
		MetaSearch[] ams = new MetaSearch[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys()
		        + getMetaForeignKeys() + getMetaCheckConstraints() + getMetaTriggers()];
		for (int iColumn = 0; iColumn < getMetaColumns(); iColumn++)
		{
		...
		{
			ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + iForeignKey] = getMetaForeignKey(
			        iForeignKey);
		}
		for (int iCheckConstraint = 0; iCheckConstraint < getMetaCheckConstraints(); iCheckConstraint++)
		{
			ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + getMetaForeignKeys()
			        + iCheckConstraint] = getMetaCheckConstraint(iCheckConstraint);
		}
		for (int iTrigger = 0; iTrigger < getMetaTriggers(); iTrigger++)
		{
			ams[getMetaColumns() + iPrimaryKeys + getMetaCandidateKeys() + getMetaForeignKeys()
			        + getMetaCheckConstraints() + iTrigger] = getMetaTrigger(iTrigger);
		}
		return ams;
	} /* getSubMetaSearches */
	...
	{
		return new String[] { getName(), String.valueOf(getRows()), getDescription() };
	} /* getSearchElements */

	/*------------------------------------------------------------------*/
	/**
	 * {@inheritDoc} toString() returns the name of the table which is to be
	 * displayed as the label of the table node of the tree displaying the archive.
	 */
	@Override
