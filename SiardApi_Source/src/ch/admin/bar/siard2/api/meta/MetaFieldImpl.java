 ...
 */
public class MetaFieldImpl extends MetaValueImpl implements MetaField {
	...
	/*------------------------------------------------------------------*/
	/**
	 * get relative internal lob folder for this field or null, if it is the field
	 ...
	 * constructor
	 * 
	 * @param mcParent, MetaColumn mcAncestor parent column meta data object of
	 *                  SIARD archive.
		...
		if (bMayBeSet) {
			if (getArchiveImpl().isMetaDataDifferent(getLobFolder(), uriLobFolder)) {
				if (uriLobFolder != null) {
					MetaDataImpl mdi = (MetaDataImpl) (getMetaTable().getTable().getParentSchema().getParentArchive()
							.getMetaData());
	...
	public MetaField getMetaField(int iField) throws IOException {
		...
		for (Iterator<String> iterField = getMetaFieldsMap().keySet().iterator(); (mf == null)
				&& iterField.hasNext();) {
	...
	public String[] getSearchElements(DU du) throws IOException {
		return new String[] { getName(), String.valueOf(getPosition()),
				getLobFolder() == null ? "" : getLobFolder().toString(), getMimeType(), getType(),
				(getCardinality() <= 0) ? "" : String.valueOf(getCardinality()), getDescription() };
	} /* getSearchElements */

	/*------------------------------------------------------------------*/
	/**
	 * {@inheritDoc} toString() returns the name of the field which is to be
