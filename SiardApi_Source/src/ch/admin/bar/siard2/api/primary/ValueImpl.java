...
/*====================================================================*/
/**
 * ValueImpl implements the interface Value (common stuff for Cell and Field).
 * 
...
public abstract class ValueImpl implements Value
{
	...
	private Element getValueElement() throws IOException
	{
		if (_elValue == null)
		{
			...
			{
				_elValue = RecordImpl.getDocument().createElementNS(Archive.sSIARD2_TABLE_NAMESPACE,
				        getColumnTag(getIndex()));
			}
			else
			{
	 ...
	 * @throws IOException if an I/O error occurred.
	 */
	protected abstract Field createField(int iField, MetaField mf, Element el) throws IOException;

	/*------------------------------------------------------------------*/
	/**
	 * return the internal LOB folder relative to this table's LOB folder, or null,
	 * if it should be stored externally.
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * return the field meta data with the given index in this column's or field's
	 * meta data.
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * return the absolute LOB folder for externally storing the LOB of this field.
	 * 
	 * @return absolute LOB folder or null, if the LOB of this column is not to be
	 *         stored externally.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * return the cardinality (maximum length of array) associated with this cell or
	 * field.
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * return the predefined type of this cell or field as a java.sql.Types integer
	 * using this mapping: null NULL "CHAR[(<Length>)]" CHAR "VARCHAR[(<Length>)]"
	 * VARCHAR "CLOB[(<LOB Length>)]" CLOB "NCHAR[(<Length>)]" NCHAR
	 * "NVARCHAR[(<Length>)]" NVARCHAR "NCLOB[(<LOB Length>)]" NCLOB "XML" SQLXML
	 * "BINARY[(<Length>)]" BINARY "VARBINARY[(<Length>)]" VARBINARY "BLOB[(<LOB
	 * Length>)]" BLOB "BOOLEAN" BOOLEAN "SMALLINT" SMALLINT "INTEGER" INTEGER
	 * "BIGINT" BIGINT "DECIMAL[(<Precision>[,<Scale>])]" DECIMAL
	 * "NUMERIC[(<Precision>[,<Scale>])]" NUMERIC "REAL" REAL "FLOAT[(<Precision>)]"
	 * FLOAT "DOUBLE PRECISION" DOUBLE "DATE" DATE "TIME[(<Scale>)]" TIME
	 * "TIMESTAMP[(<Scale>)]" TIMESTAMP "INTERVAL ..." OTHER
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * return the MIME type of the LOBs in this column or field, or null, if it was
	 * not set.
	 * 
	...
	/*------------------------------------------------------------------*/
	/**
	 * get suitable tag for the type and index of this cell's or field's child
	 * field.
	 * 
	...
	public String getLobFilename() throws IOException
	{
		...
		else if ((iPreType == Types.BINARY) || (iPreType == Types.VARBINARY) || (iPreType == Types.BLOB))
		{
	 ...
	 * @param iIndex                index of value in parent cell or field (is
	 *                              needed, because element can be null).
	 * @param cell                  parent cell if this is the value of a cell.
	 * @param elValue               DOM element representing the value or null for
	 *                              NULL value.
	 * @param mv                    meta data (column meta data for cell value,
	 *                              field meta data for field value).
	 */
	        ...
	        throws IOException
	{
		...
		if (elValue != null)
		{
			...
			if (iCardinality > 0)
			{
				...
				for (Node nodeChild = elValue.getLastChild(); (elChild == null)
				        && (nodeChild != null); nodeChild = nodeChild.getPreviousSibling())
				{
		...
		if (!isNull())
		{
			...
			else if (iPreType != Types.NULL)
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to boolean!");
			}
			else
			{
	...
	public void setBoolean(boolean b) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to boolean value!");
		}
		else
		{
	...
	public Short getShort() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.SMALLINT) || (iPreType == Types.INTEGER) || (iPreType == Types.BIGINT))
			{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to short!");
			}
			else
			{
	...
	public void setShort(short sh) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to short value!");
		}
		else
		{
	...
	public Integer getInt() throws IOException
	{
		...
		if (!isNull())
		{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to int!");
			}
			else
			{
	...
	public void setInt(int i) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to int value!");
		}
		else
		{
	...
	public BigInteger getBigInteger() throws IOException
	{
		...
		if (!isNull())
		{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to BigInteger!");
			}
			else
			{
	...
	public void setBigInteger(BigInteger bi) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to BigInteger value!");
		}
		else
		{
	...
	public BigDecimal getBigDecimal() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.DECIMAL) || (iPreType == Types.NUMERIC) || (iPreType == Types.SMALLINT)
			        || (iPreType == Types.INTEGER) || (iPreType == Types.BIGINT) || (iPreType == Types.FLOAT)
			        || (iPreType == Types.REAL) || (iPreType == Types.DOUBLE))
			{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to BigDecimal!");
			}
			else
			{
	...
	public void setBigDecimal(BigDecimal bd) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to BigDecimal value!");
		}
		else
		{
	...
	public Float getFloat() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.REAL) || (iPreType == Types.DOUBLE) || (iPreType == Types.FLOAT))
			{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to float!");
			}
			else
			{
	...
	public void setFloat(float f) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to float value!");
		}
		else
		{
	...
	public Double getDouble() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.REAL) || (iPreType == Types.FLOAT) || (iPreType == Types.DOUBLE))
			{
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to double!");
			}
			else
			{
	...
	public void setDouble(double d) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to double value!");
		}
		else
		{
	...
	public Date getDate() throws IOException
	{
		...
		if (!isNull())
		{
			...
			if (iPreType == Types.DATE)
			{
				...
				{
					throw new IllegalArgumentException(
					        "Cell value " + getValueElement().getTextContent() + " could not be parsed as xs:date!",
					        pe);
				}
			}
			else if (iPreType != Types.NULL)
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to date!");
			}
			else
			{
	...
	public void setDate(Date date) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to date value!");
		}
		else
		{
	...
	public Time getTime() throws IOException
	{
		...
		if (!isNull())
		{
			...
			if (iPreType == Types.TIME)
			{
				...
				{
					throw new IllegalArgumentException(
					        "Cell value " + getValueElement().getTextContent() + " could not be parsed as xs:time!",
					        pe);
				}
			}
			else if (iPreType != Types.NULL)
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to time!");
			}
			else
			{
	...
	public void setTime(Time time) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to time value!");
		}
		else
		{
	...
	public Timestamp getTimestamp() throws IOException
	{
		...
		if (!isNull())
		{
			...
			if (iPreType == Types.TIMESTAMP)
			{
				...
				{
					throw new IllegalArgumentException(
					        "Cell value " + getValueElement().getTextContent() + " could not be parsed as xs:dateTime!",
					        pe);
				}
			}
			else if (iPreType != Types.NULL)
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to timestamp!");
			}
			else
			{
	...
	public void setTimestamp(Timestamp ts) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to timestamp value!");
		}
		else
		{
	...
	public Duration getDuration() throws IOException
	{
		...
		if (!isNull())
		{
			...
			if (iPreType == Types.OTHER)
			{
				...
				{
					throw new IllegalArgumentException(
					        "Cell value " + getValueElement().getTextContent() + " could not be parsed as xs:duration!",
					        pe);
				}
			}
			else if (iPreType != Types.NULL)
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be converted to duration!");
			}
			else
			{
	...
	public void setDuration(Duration duration) throws IOException
	{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set to duration value!");
		}
		else
		{
	...
	public Reader getReader() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.CHAR) || (iPreType == Types.VARCHAR) || (iPreType == Types.CLOB)
			        || (iPreType == Types.NCHAR) || (iPreType == Types.NVARCHAR) || (iPreType == Types.NCLOB)
			...
			{
				throw new IllegalArgumentException(
				        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be read from input stream!");
			}
			else
			{
	...
	/** {@inheritDoc} */
	@Override
	public void setReader(Reader rdrClob) throws IOException
	{
		int iPreType = getPreType();
		if ((iPreType == Types.CHAR) || (iPreType == Types.VARCHAR) || (iPreType == Types.CLOB)
		        || (iPreType == Types.NCHAR) || (iPreType == Types.NVARCHAR) || (iPreType == Types.NCLOB)
		        || (iPreType == Types.SQLXML))
		{
			...
			int				iOffset			= 0;
			int				iRead			= -1;
			for (iRead = rdrClob.read(cbufPrefix); (iOffset < cbufPrefix.length)
			        && (iRead != -1); iRead = rdrClob.read(cbufPrefix, iOffset, cbufPrefix.length - iOffset))
			{
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set using a reader!");
		}
		else
		{
	...
	public InputStream getInputStream() throws IOException
	{
		...
		{
			int iPreType = getPreType();
			if ((iPreType == Types.BINARY) || (iPreType == Types.VARBINARY) || (iPreType == Types.BLOB))
			{
				if (getValueElement().hasAttribute(ArchiveImpl._sATTR_FILE))
				{
					String	sLobFile				= getValueElement().getAttribute(ArchiveImpl._sATTR_FILE);
	...
	/** {@inheritDoc} */
	@Override
	public void setInputStream(InputStream isBlob) throws IOException
	{
		int iPreType = getPreType();
		if ((iPreType == Types.BINARY) || (iPreType == Types.VARBINARY) || (iPreType == Types.BLOB))
		{
			...
			int				iOffset			= 0;
			int				iRead			= -1;
			for (iRead = isBlob.read(bufPrefix); (iOffset < bufPrefix.length)
			        && (iRead != -1); iRead = isBlob.read(bufPrefix, iOffset, bufPrefix.length - iOffset))
			{
				iOffset = iOffset + iRead;
			}
			String	sLobFile				= getLobFilename();
		...
		{
			throw new IllegalArgumentException(
			        "Cell of type " + SqlTypes.getTypeName(iPreType) + " cannot be set using an input stream!");
		}
		else
		{
