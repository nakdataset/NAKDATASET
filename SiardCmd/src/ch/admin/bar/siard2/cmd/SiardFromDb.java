...
public class SiardFromDb
{
	...
	private static IndentLogger				_il						= IndentLogger
	        .getIndentLogger(SiardFromDb.class.getName());
	/** manifest */
	...
	private static ProgramInfo				_pi						= ProgramInfo.getProgramInfo("SIARD Suite",
	        MF.getSpecificationVersion(), "SiardFromDb", MF.getImplementationVersion(),
	...
	private void printUsage()
	{
		...
		System.out.println("  <login timeout>      login timeout in seconds (default: "
		        + String.valueOf(_iLoginTimeoutSeconds) + "), 0 for unlimited");
		System.out.println("  <query timeout>      query timeout in seconds (default: "
		        + String.valueOf(_iQueryTimeoutSeconds) + "), 0 for unlimited");
		...
		for (int i = 0; i < asSchemes.length; i++)
		{
			...
			System.out.println("for " + sc.getTitle(sScheme) + ": "
	...
	private void getParameters(String[] asArgs)
	{
		...
		if (_iReturn == iRETURN_OK)
		{
			...
			if (sExternalLobFolder != null)
			{
				...
				{
					System.out.println(
					        "External LOB folder  " + _fileExternalLobFolder.getAbsolutePath() + " does not exist!");
					_iReturn = iRETURN_ERROR;
				}
				else
				{
					/* relativize it from SIARD file's parent folder */
					File fileRelative = _fileSiard.getParentFile().getAbsoluteFile().toPath()
					        .relativize(_fileExternalLobFolder.getAbsoluteFile().toPath()).toFile();
					/*
					 * prepend a ../ for exiting the SIARD file and append a / to indicate that it
					 * is a folder
					 */
					try
					{
					...
					{
						System.out.println("External LOB folder  " + _fileExternalLobFolder.getAbsolutePath()
						        + " could not be relativized!");
						_iReturn = iRETURN_ERROR;
					}
				}
	...
	SiardFromDb(String asArgs[]) throws SQLException, IOException, ClassNotFoundException
	{
		super();
		_il.enter();
		/* parameters */
		...
		if (_iReturn == iRETURN_OK)
		{
			...
			{
				/* open connection */
				System.out.println("SiardFromDb 342 SiardConnection.getSiardConnection before");
				String sError = SiardConnection.getSiardConnection().loadDriver(_sJdbcUrl);
				System.out.println("SiardFromDb 342 SiardConnection.getSiardConnection after");
				
				
				if ((sError == null) || (sError.length() == 0))
				{
					System.out.println("SiardFromDb 349 DriverManager.setLoginTimeout");
					DriverManager.setLoginTimeout(_iLoginTimeoutSeconds);
					System.out.println("SiardFromDb 351 DriverManager.setLoginTimeout");
					_conn = DriverManager.getConnection(_sJdbcUrl, _sDatabaseUser, _sDatabasePassword);
					System.out.println("SiardFromDb 353 DriverManager.setLoginTimeout");
					...
					if ((_conn != null) && (!_conn.isClosed()))
					{
						...
						if (_uriExternalLobFolder != null)
						{
							...
							if (mcMaxLob != null)
							{
								...
								String sMessage = "LOBs in database column \"" + sColumnName + "\" in table \""
								        + sTableName + "\" in schema \"" + sSchemaName
								        + "\" will be stored externally in folder \""
								        + _fileExternalLobFolder.getAbsolutePath().toString() + "\"";
