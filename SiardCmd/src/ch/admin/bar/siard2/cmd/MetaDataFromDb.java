...
public class MetaDataFromDb extends MetaDataBase
{
	...
	static final Pattern		_patARRAY_CONSTRUCTOR	= Pattern
	        .compile("^\\s*(.*?)\\s+ARRAY\\s*\\[\\s*(\\d+)\\s*\\]$");
	...
	/*
	 * ==================================================================== Start
	 * global metadata dump and check.
	 * ====================================================================
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * increment the number or tables analyzed, issuing a notification, when a
	 * percent is reached.
	 */
				...
				else if (iDataType == Types.ARRAY)
				{
					...
					if (m.matches())
					{
						...
						else
						{
							...
							QualifiedId	qiTypeBase	= new QualifiedId(null, mtyBase.getParentMetaSchema().getName(),
							        mtyBase.getName());
					...
					{
						throw new SQLException("Invalid ARRAY constructor for attribute " + ma.getName() + " of type "
						        + mt.getName() + "!");
					}
				}
				else
	 ...
	 * @throws SQLException if a database error occurred.
	 */
	private MetaType createType(String sTypeName, String sDefaultSchema, int iPrecision, int iScale)
	        throws IOException, SQLException
	{
		...
		try
		{
			...
			if (mt == null)
			{
				...
				ResultSet rs = _dmd.getUDTs(null, ((BaseDatabaseMetaData) _dmd).toPattern(qiType.getSchema()),
			...
			if (sSpecificName.equals(mr.getSpecificName()))
			{
				...
				else
				{
					...
					else if (iDataType == Types.ARRAY)
					{
						...
						{
							throw new SQLException("Invalid ARRAY constructor for parameter " + mp.getName()
							        + " of routine " + mr.getName() + "!");
						}
					}
					else
					{
	...
	private void getFunctionParameters(MetaRoutine mr) throws IOException, SQLException
	{
		...
		while (rs.next())
		{
			...
			{
				/*
				 * the functionColumn... constants are defined inconsistently and probably were
				 * supposed to be identical with the corresponding procedureColumn... constants.
				 */
				if ((iColumnType == DatabaseMetaData.procedureColumnReturn)
				        || (iColumnType == DatabaseMetaData.procedureColumnResult))
				{
	...
	private void getColumnData(ResultSet rs, MetaColumn mc) throws IOException, SQLException
	{
		...
		else if (iDataType == Types.ARRAY)
		{
			...
			{
				throw new SQLException("Invalid ARRAY constructor for column " + mc.getName() + " of table "
				        + qiParent.format() + "!");
			}
		}
		else
	...
	private void getPrivileges() throws IOException, SQLException
	{
		for (int iSchema = 0; iSchema < _md.getMetaSchemas(); iSchema++)
		{
			...
			for (int iTable = 0; iTable < ms.getMetaTables(); iTable++)
			{
				...
				ResultSet	rs	= _dmd.getTablePrivileges(null, ((BaseDatabaseMetaData) _dmd).toPattern(ms.getName()),
				        ((BaseDatabaseMetaData) _dmd).toPattern(mt.getName()));
				while (rs.next())
				{
					String			sTableSchema	= rs.getString("TABLE_SCHEM");
					...
					MetaPrivilege	mp				= _md.createMetaPrivilege(sPrivilege,
					        (new QualifiedId(null, sTableSchema, sTableName)).format(), sGrantor, sGrantee);
	...
	{
		ResultSet rs = _dmd.getTables(null, ((BaseDatabaseMetaData) _dmd).toPattern(ms.getName()), "%",
		        new String[] { "VIEW" });
		while (rs.next())
		{
			String sTableSchema = rs.getString("TABLE_SCHEM");
			...
			{
				System.err.println(
				        "View " + qiView.format() + " could not be archived (" + EU.getExceptionMessage(se) + ")!");
	...
	/*------------------------------------------------------------------*/
	/**
	 * query the number of rows and the sizes of the LOB columns and update the LOB
	 * column of maximum size.
	 * 
	 * @param mt
	 * @throws IOException  if an I/O error occurred.
	 * @throws SQLException if a database error occurred.
		...
		if (_bMaxLobNeeded)
		{
			for (int iColumn = 0; iColumn < mt.getMetaColumns(); iColumn++)
			{
				...
				if ((mc.getCardinality() < 0)
				        && ((iPreType == Types.CLOB) || (iPreType == Types.NCLOB) || (iPreType == Types.BLOB)
						// || (iPreType == Types.SQLXML) || // DB/2 stores XML as a hierarchical object
						// tree ...
						))
				{
					if (!mc.getTypeOriginal().equals("\"LONG\"")) // Oracle idiocy
					{
						sQuery = sQuery + ",\r\n SUM(OCTET_LENGTH(" + SqlLiterals.formatId(mc.getName()) + "))" + " AS "
						        + SqlLiterals.formatId(mc.getName() + "_SIZE");
						iLobs++;
					}
				}
			}
	...
	private void getUniqueKeys(MetaTable mt) throws IOException, SQLException
	{
		...
		ResultSet				rs					= _dmd.getIndexInfo(null, mt.getParentMetaSchema().getName(),
		        mt.getName(), true, false);
		while (rs.next())
		{
			String sTableSchema = rs.getString("TABLE_SCHEM");
			...
			if ((iIndexType != DatabaseMetaData.tableIndexStatistic) && (iIndexType != DatabaseMetaData.tableIndexOther)
			        && (!bPrimary))
			{
	...
	private void getForeignKeys(MetaTable mt) throws IOException, SQLException
	{
		...
		ResultSet				rs				= _dmd.getImportedKeys(null, mt.getParentMetaSchema().getName(),
		        mt.getName());
		while (rs.next())
		{
	...
	private void getPrimaryKey(MetaTable mt) throws IOException, SQLException
	{
		...
		ResultSet				rs				= _dmd.getPrimaryKeys(null, mt.getParentMetaSchema().getName(),
		        mt.getName());
		while (rs.next())
		{
			String sTableSchema = rs.getString("TABLE_SCHEM");
	 ...
	 */
	private MetaDataFromDb(DatabaseMetaData dmd, MetaData md, Archive a) throws SQLException
	{
	...
	public static MetaDataFromDb newInstance(DatabaseMetaData dmd, MetaData md, Archive a) throws SQLException
	{		
