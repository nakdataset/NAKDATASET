...
public class MetaDataToDb extends MetaDataBase
{
	...
	/*------------------------------------------------------------------*/
	/**
	 * increment the number or tables created, issuing a notification, when a
	 * percent is reached.
	 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * create an attribute definition for a CREATE TYPE statement from attribute
	 * meta data.
	 * 
	...
	private String createAttribute(MetaAttribute ma, TypeMapping tm)
	{
		...
		else
		{
			...
			QualifiedId qiType = new QualifiedId(null, sm.getMappedSchemaName(),
			        sm.getMappedTypeName(ma.getTypeName()));
	...
	private boolean existsType(String sMangledSchema, String sMangledType) throws SQLException
	{
		...
		ResultSet rs = _dmd.getUDTs(null, ((BaseDatabaseMetaData) _dmd).toPattern(sMangledSchema),
		        ((BaseDatabaseMetaData) _dmd).toPattern(sMangledType), new int[] { Types.STRUCT, Types.DISTINCT });
		while (rs.next())
			bExists = true;
		rs.close();
		return bExists;
	...
	private String createColumn(MetaColumn mc, TableMapping tm) throws IOException
	{
		...
		else
		{
			...
			else
			{
				...
				QualifiedId qiType = new QualifiedId(null, sm.getMappedSchemaName(),
				        sm.getMappedTypeName(mc.getTypeName()));
				sbSql.append(qiType.format());
			}
		}
		if (!mc.isNullable())
	...
	private void createTable(MetaTable mt, SchemaMapping sm) throws IOException, SQLException
	{
		...
		ResultSet rsColumns = _dmd.getColumns(null, ((BaseDatabaseMetaData) _dmd).toPattern(qiTable.getSchema()),
	...
	{
		boolean bExists = false;
		ResultSet rs = _dmd.getTables(null, ((BaseDatabaseMetaData) _dmd).toPattern(sMangledSchema),
		        ((BaseDatabaseMetaData) _dmd).toPattern(sMangledTable), new String[] { "TABLE" });
		if (rs.next())
			bExists = true;
		rs.close();
		return bExists;
	...
	public void upload(Progress progress) throws IOException, SQLException
	{
		...
		for (int iSchema = 0; (iSchema < _md.getMetaSchemas()) && (!cancelRequested()); iSchema++)
		{
			...
			if (existsSchema(sm.getMappedSchemaName()))
			{
				...
				throw new SQLException("Schema \"" + sm.getMappedSchemaName() + "\" could not be created! " + "Map \""
				        + ms.getName() + "\" to existing schema.");
		}
		if (cancelRequested())
	...
	/*------------------------------------------------------------------*/
	/**
	 * returns the number of tables that exist in the database and will be dropped,
	...
	/*------------------------------------------------------------------*/
	/**
	 * matchAttributes returns true, if the attributes of the given type meta data
	 * match the attributes in the database.
	 * 
	 * @param mt type meta data.
	 * @param sm mapping of names in schema.
	 * @return true, if the attributes of the type in the database match the
	 *         attributes in the meta data.
	 * @throws IOException  if an I/O error occurred
	 * @throws SQLException if a database error occurred.
	 */
	private boolean matchAttributes(MetaType mt, SchemaMapping sm) throws IOException, SQLException
	{
		...
		TypeMapping tm = sm.getTypeMapping(mt.getName());
		int iPosition = 0;
		ResultSet rs = _dmd.getAttributes(null, ((BaseDatabaseMetaData) _dmd).toPattern(sm.getMappedSchemaName()),
		...
		while (bMatches && rs.next())
		{
			...
			if (ma != null)
			{
				...
				else
				{
					...
					{
						throw new SQLException("Type of attribute " + ma.getName() + " of type " + mt.getName()
						        + " could not be parsed!", pe);
					}
				}
			}
			else
	...
	/*------------------------------------------------------------------*/
	/**
	 * matchType returns true, if a type of the given name exists in the database
	 * and has the same relevant properties.
	 * 
	 * @param mt type meta data.
	 * @param sm mapping of names in schema.
	...
	private boolean matchType(MetaType mt, SchemaMapping sm) throws IOException, SQLException
	{
		...
		ResultSet rs = _dmd.getUDTs(null, ((BaseDatabaseMetaData) _dmd).toPattern(sm.getMappedSchemaName()),
		        ((BaseDatabaseMetaData) _dmd).toPattern(tm.getMappedTypeName()), null);
		while (rs.next())
		{
			/*
			 * the type only needs to be dropped if its base type or its attributes are
			 * different
			 */
	...
	/*------------------------------------------------------------------*/
	/**
	 * returns the number of types that exist in the database and will be dropped,
	 * when upload is executed.
	 * 
	 * @return number of tables that will be dropped, when upload is executed.
	 * @throws IOException  if an I/O error occurred
	        ...
	        throws IOException, SQLException
	{
		...
		_am = ArchiveMapping.newInstance(supportsArrays(), supportsUdts(), mapSchemas, _md, _iMaxTableNameLength,
		        _iMaxColumnNameLength);
	} /* constructor */
