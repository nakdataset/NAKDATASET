...
public class PrimaryDataToDb extends PrimaryDataTransfer
{
	...
	/*------------------------------------------------------------------*/
	/**
	 * increment the number or records uploaded, issuing a notification, when a
	 * percent is reached.
	 */
	...
	public void addForeignKeys(Connection conn, MetaTable mt) throws SQLException
	{
		...
		if (mt.getMetaForeignKeys() > 0)
		{
			...
			for (int iForeignKey = 0; iForeignKey < mt.getMetaForeignKeys(); iForeignKey++)
			{
				...
				QualifiedId qiReferenced = new QualifiedId(null, smReferenced.getMappedSchemaName(),
				        tmReferenced.getMappedTableName());
	...
	/*------------------------------------------------------------------*/
	/**
	 * try to enable all table constraints. displays warnings on failure but
	 * continues. (Rationale: DB systems differ too much in constraint handling.)
	 */
	 ...
	 */
	private Object getValue(Value value, Connection conn, Set<Object> setResources) throws IOException, SQLException
	{
		Object o = null;
		if (!value.isNull())
		{
	...
	private void putTable(Table table, SchemaMapping sm) throws IOException, SQLException
	{
		...
		while ((lRecord < mt.getRows()) && (!cancelRequested()))
		{
			...
			/***
			 * ResultSetMetaData rsmd = rs.getMetaData(); for (int i = 0; i <
			 * rsmd.getColumnCount(); i++) { Object o = rs.getObject(i+1);
			 * System.out.println(String.valueOf(i)+".
			 * "+rsmd.getColumnName(i+1)+"/"+rsmd.getColumnLabel(i+1)+": "+o.toString()); }
			 ***/
			...
			if ((lRecord % lCOMMIT_RECORDS) == 0)
			{
				System.out.println("    Record " + String.valueOf(lRecord) + " ("
				        + sw.formatRate(rd.getByteCount() - lBytesStart, sw.stop()) + " kB/s)");
		...
		System.out.println("    Record " + String.valueOf(lRecord) + " ("
		        + sw.formatRate(rd.getByteCount() - lBytesStart, sw.stop()) + " kB/s)");
		System.out.println("    Total: " + StopWatch.formatLong(lRecord) + " records ("
		        + StopWatch.formatLong(rd.getByteCount()) + " bytes in " + sw.formatMs() + " ms)\r\n\r\n");
		if (!rs.isClosed())
			rs.close();
		if (!stmt.isClosed())
			stmt.close();
	 ...
	 * @throws SQLException if a database error occurred.
	 */
	private PrimaryDataToDb(Connection conn, Archive archive, ArchiveMapping am, boolean bSupportsArrays,
	        boolean bSupportsDistincts, boolean bSupportsUdts) throws SQLException
	{
	 ...
	 * @throws SQLException if a database error occurred.
	 */
	public static PrimaryDataToDb newInstance(Connection conn, Archive archive, ArchiveMapping am,
	        boolean bSupportsArrays, boolean bSupportsDistincts, boolean bSupportsUdts) throws SQLException
	{
