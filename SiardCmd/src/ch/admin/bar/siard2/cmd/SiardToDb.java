...
public class SiardToDb
{
	...
	private static ProgramInfo				_pi						= ProgramInfo.getProgramInfo("SIARD Suite",
	        MF.getSpecificationVersion(), "SiardToDb", MF.getImplementationVersion(),
	...
	private void printUsage()
	{
		...
		System.out.println("  <login timeout>     login timeout in seconds (default: "
		        + String.valueOf(_iLoginTimeoutSeconds) + ")");
		System.out.println("  <query timeout>     query timeout in seconds (default: "
		        + String.valueOf(_iQueryTimeoutSeconds) + ")");
	...
	SiardToDb(String asArgs[]) throws IOException, SQLException
	{
		super();
		_il.enter();
		/* parameters */
			...
			if ((sError == null) || (sError.length() == 0))
			{
				...
				if ((_conn != null) && (!_conn.isClosed()))
				{
					...
					if (_bOverwrite || ((mdtd.tablesDroppedByUpload() == 0) && (mdtd.typesDroppedByUpload() == 0)))
					{
						...
						PrimaryDataToDb pdtd = PrimaryDataToDb.newInstance(_conn, _archive, mdtd.getArchiveMapping(),
						        mdtd.supportsArrays(), mdtd.supportsDistincts(), mdtd.supportsUdts());
